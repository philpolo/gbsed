...
## Dependencies on `roadscene2vec`

This project relies on the [roadscene2vec](https://github.com/AICPS/roadscene2vec) library for scene graph extraction, dataset management, and graph-based learning. The library must be installed and available in the Python path. The `roadscene2vec` module is imported via `sys.path` manipulation in the source files; ensure it is located at the expected relative path (`../../roadscene2vec` or installed as a package).

---

## Citation

If you use this work, please cite the corresponding publication (details to be added upon publication).
